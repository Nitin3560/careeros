...
from app.ingestion.poller.normalization import html_to_text, source_description_html  # noqa: E402
    ...
    while args.limit is None or processed < args.limit:
        ...
        try:
            rows = db.execute(text("""
                SELECT id, source, title, location, description_text, raw_payload
                FROM jobs
                WHERE description_status = 'pending'
                ORDER BY id LIMIT :limit FOR UPDATE SKIP LOCKED
            """), {"limit": size}).mappings().all()
            if not rows:
                db.rollback()
                break
            updates = []
            for row in rows:
                if row["raw_payload"]:
                    ...
                    description_html = row["description_text"] or ""
                    description_text = html_to_text(description_html)
            ...
            db.execute(text("""
                UPDATE jobs SET description_html=:html, description_text=:text,
                    content_hash=:hash, description_status=:status WHERE id=:id
            """), updates)
            db.commit()
            processed += len(rows)
