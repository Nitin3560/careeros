...
def main():
    ...
    try:
        ...
        for row in db.execute(text("SELECT sponsorship_evidence,count(*) FROM jobs WHERE sponsorship_block AND sponsorship_evidence IS NOT NULL GROUP BY 1 ORDER BY 2 DESC LIMIT 30")):
            print(row[1], row[0])
    finally: db.close()
