...
def binary_stats(pairs):
    tp=sum(a==b=='y' for a,b in pairs); fp=sum(a=='n' and b=='y' for a,b in pairs); fn=sum(a=='y' and b=='n' for a,b in pairs); tn=sum(a==b=='n' for a,b in pairs); support=tp+fn
    ...
    return {'precision':precision,'recall':recall,'f1':f1,'support':support,'confusion':{'tp':tp,'fp':fp,'tn':tn,'fn':fn}}
def score_rows(rows, predictions):
    ...
    for field in ('is_tech','is_senior','location_class','sponsorship_block','min_years'):
        ...
        for row,pred in zip(rows,predictions):
            ...
            if field in ('is_tech','is_senior','sponsorship_block'): expected={'y':'y','n':'n'}[expected]; got='y' if got else 'n'; pairs.append((expected,got))
        ...
        if field=='location_class':
            ...
            per_class={}
            for label in labels:
                tp=matrix[label][label]; fp=sum(matrix[other][label] for other in labels if other!=label); fn=sum(matrix[label][other] for other in labels if other!=label)
                precision=tp/(tp+fp) if tp+fp else 0; recall=tp/(tp+fn) if tp+fn else 0
                per_class[label]={'precision':precision,'recall':recall,'f1':2*precision*recall/(precision+recall) if precision+recall else 0,'support':sum(matrix[label].values())}
            report[field]={'support':len(pairs),'confusion_matrix':matrix,'per_class':per_class}
